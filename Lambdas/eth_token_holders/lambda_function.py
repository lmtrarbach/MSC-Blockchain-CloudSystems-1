    ...
    try:
        ...
        sql_query_to_execute = f'''
            SELECT 
                ...
                COALESCE(SUM(received.value), 0) - COALESCE(SUM(sent.value), 0) AS total_holdings
            FROM (
                SELECT to_address, SUM(value) AS value
                FROM token_transfers
                WHERE date >= '{date}' AND token_address = '{token_address}'
                GROUP BY to_address
            ) AS received
            LEFT JOIN (
                SELECT from_address, SUM(value) AS value
                FROM token_transfers
                WHERE date >= '{date}' AND token_address = '{token_address}'
